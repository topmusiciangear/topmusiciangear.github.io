...
`;
  ...
  const storeMeta = {
    ...
    'Music Store': { color: '#555555', mark: '<img src="img/musicstore-icon.png" alt="Music Store">' },
    'zZounds': { color: '#1a3a5c', mark: '<img src="img/zzounds-icon.png" alt="zZounds">' }
  };
  const slugToName = {
    andertons: 'Andertons',
  ...
  function affWrap(store, url) {
    ...
    switch (store) {
      ...
      case "musicstore":
        return url.indexOf("awin1.com") > -1 ? url : "https://www.awin1.com/cread.php?awinmid=63816&awinaffid=2891111&ued=" + encodeURIComponent(url);
      case "reverb":
        return url.indexOf("awin1.com") > -1 ? url : "https://www.awin1.com/cread.php?awinmid=67144&awinaffid=2891111&ued=" + encodeURIComponent(url);
      case "zzounds":
        return url.indexOf("anrdoezrs.net") > -1 ? url : "https://www.anrdoezrs.net/click-101857888-10422044-1779394?url=" + encodeURIComponent(url);
      case "pluginboutique":
        return /a_aid=/.test(url) ? url : url + q + "a_aid=6a01e859cbe1a";
      case "amazon":
        return /(\?|&)tag=/.test(url) ? url : url + q + "tag=topmusicg-20";
      default:
        return url;
    }
  }

  const cjz = (u) => /zzounds\.com/.test(u) && u.indexOf('anrdoezrs.net') < 0 ? 'https://www.anrdoezrs.net/click-101857888-10422044-1779394?url=' + encodeURIComponent(u.replace('/a--925521', '')) : u;
  ...
  const dealCard = (d) => {
    ...
    const storeLink = `<a href="${cjz(affWrap(d.store, d.store_url))}" target="_blank" rel="noopener noreferrer sponsored" class="deal-store-tag" style="--store-color:${sc.color}">${sc.mark}<span>${storeName}</span></a>`;
    return `
  <div class="deals-card deal-card">
    <div class="deal-store-row">${storeLink}</div>
    <div class="deal-card-top">
      <a class="deal-img" href="${cjz(affWrap(d.store, d.store_url))}" target="_blank" rel="noopener noreferrer sponsored"><img src="${d.img}" alt="${d.title}" loading="lazy"></a>
      <div class="deal-body">
        ...
        <a href="${cjz(affWrap(d.store, d.store_url))}" target="_blank" rel="noopener noreferrer sponsored" class="deals-store-btn">★ ${t('View Deal', 'Ver Oferta')}</a>
      </div>
    </div>
  </div>`;
  };
...
<head>
  ...
  <style>
    ...
    .deal-badge { display: inline-flex; align-items: center; padding: 0 7px; border-radius: 10px; background: rgba(34,197,94,.15); border: 1px solid rgba(34,197,94,.4); color: #22c55e; font-size: 10px; font-weight: 700; line-height: 1; white-space: nowrap; box-sizing: border-box; height: 16px; }
