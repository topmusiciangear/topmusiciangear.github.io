...
const buildPage = (lang) => {
  ...
  const navDropdown = `<div class="nav-dd">
    ...
    <div class="nav-dd-panel" role="menu">
      ...
      <a class="nav-dd-link" href="https://www.anrdoezrs.net/click-101857888-10422044?url=https%3A%2F%2Fwww.zzounds.com%2Fa--925521%2Flp%2Fnew-products%2F219" target="_blank" rel="noopener noreferrer sponsored"><img src="img/zzounds-icon.png" alt="" class="nav-dd-link-icon">zZounds</a>
    </div>
  </div>`;
...
`;
  ...
  const cjz = (u) => /zzounds\.com/.test(u) ? 'https://www.anrdoezrs.net/click-101857888-10422044?url=' + encodeURIComponent(u) : u;
  ...
  const dealCard = (d) => {
    ...
    const storeLink = `<a href="${cjz(d.store_url)}" target="_blank" rel="noopener noreferrer sponsored" class="deal-store-tag" style="--store-color:${sc.color}">${sc.mark}<span>${storeName}</span></a>`;
    return `
  <div class="deals-card deal-card">
    <div class="deal-store-row">${storeLink}</div>
    <div class="deal-card-top">
      <a class="deal-img" href="${cjz(d.store_url)}" target="_blank" rel="noopener noreferrer sponsored"><img src="${d.img}" alt="${d.title}" loading="lazy"></a>
      <div class="deal-body">
        ...
        <a href="${cjz(d.store_url)}" target="_blank" rel="noopener noreferrer sponsored" class="deals-store-btn">★ ${t('View Deal', 'Ver Oferta')}</a>
      </div>
    </div>
  </div>`;
  };
