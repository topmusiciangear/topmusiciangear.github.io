...
let guidePageSize = 20;
let guideVisibleCount = 20;
...
var faqBase = {
  ...
  guitars: [
    ...
    { q: "How much should I spend on a first guitar?", q_es: "¿Cuánto debería gastar en mi primera guitarra?", a: "A budget of $200-$500 is the sweet spot for a quality beginner guitar. The Yamaha Pacifica 112V ($299) and Squier Affinity Stratocaster ($299) are excellent starters that won't hold you back. Avoid guitars under $150 as they often have playability issues that frustrate beginners.", a_es: "Un presupuesto de $200-$500 es el punto óptimo para una guitarra de principiante de calidad. La Yamaha Pacifica 112V ($299) y Squier Affinity Stratocaster ($299) son excelentes opciones que no te limitarán. Evita guitarras de menos de $150, ya que suelen tener problemas de tocabilidad." },
    { q: "What guitar amp should a beginner buy?", q_es: "¿Qué amplificador de guitarra debería comprar un principiante?", a: "A modeling amp like the Boss Katana 50 ($279) is the best choice for beginners, offering dozens of effects and amp models in one affordable package. For practice, the Fender Mustang LT25 ($149) is excellent. Tube amps like the Marshall DSL40CR ($749) are for intermediate to advanced players.", a_es: "Un amplificador de modelado como el Boss Katana 50 ($279) es la mejor opción para principiantes, ofreciendo docenas de efectos y modelos de amplificador en un paquete asequible. Para práctica, el Fender Mustang LT25 ($149) es excelente." },
  ...
  amps: [
    { q: "What is the best guitar amp for beginners?", q_es: "¿Cuál es el mejor amplificador de guitarra para principiantes?", a: "The Boss Katana 50 ($279) is the best modeling amp for beginners, offering dozens of effects and amp models in one affordable package. The Fender Mustang LT25 ($149) is excellent for practice. Tube amps like the Marshall DSL40CR ($749) are for intermediate to advanced players.", a_es: "El Boss Katana 50 ($279) es el mejor amplificador de modelado para principiantes. El Fender Mustang LT25 ($149) es excelente para práctica. Los amplificadores a válvulas como el Marshall DSL40CR ($749) son para niveles intermedios y avanzados." },
...
function setLang(lang) {
  ...
  var s=document.getElementById('ls');if(s)s.textContent='.'+(lang==='es'?'lang-show-en':'lang-show-es')+',.'+(lang==='es'?'cookie-lang-en':'cookie-lang-es')+'{display:none!important}';
  translatePage();
  renderGuideCats();
  if (currentGuideId) {
...
function openGuideVideo(v) {
  ...
  var lb = document.createElement("div");
  lb.id = "lightbox";
  lb.style.cssText = "display:flex;position:fixed;inset:0;z-index:99999;align-items:center;justify-content:center;padding:16px 12px;box-sizing:border-box;background:rgba(0,0,0,.92)";
  var w = document.createElement("div");
  w.style.cssText = "position:relative;width:min(1100px,94vw);aspect-ratio:16/9";
  w.innerHTML = '<iframe src="https://www.youtube.com/embed/' + v + '?autoplay=1&rel=0" style="position:absolute;inset:0;width:100%;height:100%;border:0;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,.6)" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture;web-share" allowfullscreen title="Video"></iframe>';
  var close = document.createElement("button");
  close.innerHTML = "&times;";
  close.setAttribute("aria-label", "Close");
...
document.addEventListener("click", function(e) {
  ...
  var v = t.closest ? t.closest(".lb-video") : null;
  if (v && v.dataset.yt) {
    openGuideVideo(v.dataset.yt);
  }
});
...
function getFilteredGuides() {
  ...
  if (searchQuery.trim()) {
    ...
    filtered = filtered.filter(g => {
      ...
      if ((g.title_es || "").toLowerCase().includes(q)) return true;
      if (g.intro.toLowerCase().includes(q)) return true;
      if ((g.intro_es || "").toLowerCase().includes(q)) return true;
      if (g.sections.some(s => s.heading.toLowerCase().includes(q) || s.content.toLowerCase().includes(q) || (s.heading_es || "").toLowerCase().includes(q) || (s.content_es || "").toLowerCase().includes(q))) return true;
      if ((getCatName(g.category) || g.category).toLowerCase().includes(q)) return true;
      var guideProductIds = [];
      if (g.products) guideProductIds = guideProductIds.concat(g.products);
      if (g.featuredProducts) guideProductIds = guideProductIds.concat(g.featuredProducts);
      if (g.sections) g.sections.forEach(function(s) { if (s.products) guideProductIds = guideProductIds.concat(s.products); });
      var uniqueIds = [...new Set(guideProductIds)];
      return uniqueIds.some(function(pid) {
        var p = products.find(function(x) { return x.id === pid; });
        if (!p) return false;
        return p.title.toLowerCase().includes(q) || (p.title_es || "").toLowerCase().includes(q) || (p.brand || "").toLowerCase().includes(q);
      });
    });
  }
  return filtered;
...
function getResolvedStores(product) {
  ...
  allStoreKeys.forEach(key => {
    ...
    } else if (!isMacOnly && searchUrls[key]) {
...
function renderGuideGrid(fanFrom) {
  ...
  if (heroTelegram) heroTelegram.style.display = "";
  const mySetup = document.querySelector(".my-setup");
  if (mySetup) mySetup.style.display = "";
  ...
  if (filtered.length === 0) {
    ...
    const hideBtn = document.getElementById("backToGuidesBtn");
  ...
  if (btn) {
    btn.style.display = (currentCategory === "all" && filtered.length <= guideVisibleCount) ? "" : "none";
  }
}
...
function getGuideFaqs(guide) {
  ...
  if (guide.featuredSnippet && guide.featuredSnippet.faq_q1_en) {
    return [1,2,3,4,5].map(function(i) {
      return {
        q: guide.featuredSnippet['faq_q' + i + '_en'],
        q_es: guide.featuredSnippet['faq_q' + i + '_es'],
        a: guide.featuredSnippet['faq_a' + i + '_en'],
        a_es: guide.featuredSnippet['faq_a' + i + '_es']
      };
...
function getAmazonDomain() {
  try {
    ...
    var uk = ['london','dublin','belfast','edinburgh','eire','bst','gmt','gb','guernsey','jersey','man'];
    for (var i = 0; i < uk.length; i++) { if (tz.indexOf(uk[i]) !== -1) return 'co.uk'; }
  } catch(e) {}
  try {
    var lang = (navigator.language || navigator.userLanguage || '').toLowerCase();
    if (lang === 'en-gb') return 'co.uk';
  } catch(e) {}
  try {
    var langs = navigator.languages || [];
    for (var i = 0; i < langs.length; i++) {
      if (langs[i].toLowerCase() === 'en-gb') return 'co.uk';
    }
  } catch(e) {}
  return 'com';
}
function renderMySetup() {
  ...
  if (!container) return;
  var dom = getAmazonDomain();
  const gear = [
    { icon: '<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="5" width="22" height="14" rx="2"/><rect x="4" y="9" width="3" height="6" rx="0.8" fill="currentColor" opacity="0.6"/><circle cx="14" cy="12" r="3"/><circle cx="14" cy="12" r="1.2" fill="currentColor"/><rect x="19" y="10" width="1.5" height="4" rx="0.5" fill="currentColor" opacity="0.6"/></svg>', title: "Universal Audio Apollo Twin X", descKey: "setupItem1Desc", asin: "B07Y46QFLQ" },
    { icon: '<svg data-fa="headphones" class="icon fa-solid fa-headphones" viewBox="0 0 512 512" width="1em" height="1em" fill="currentColor"><path d="M256 80C149.9 80 62.4 159.4 49.6 262c9.4-3.8 19.6-6 30.4-6c26.5 0 48 21.5 48 48V432c0 26.5-21.5 48-48 48c-44.2 0-80-35.8-80-80V384 336 288C0 146.6 114.6 32 256 32s256 114.6 256 256v48 48 16c0 44.2-35.8 80-80 80c-26.5 0-48-21.5-48-48V304c0-26.5 21.5-48 48-48c10.8 0 21 2.1 30.4 6C449.6 159.4 362.1 80 256 80z"/></svg>', title: "Beyerdynamic DT 770 Pro", descKey: "setupItem2Desc", asin: "B0016MNAAI" },
    { icon: '<svg data-fa="microphone" class="icon fa-solid fa-microphone" viewBox="0 0 384 512" width="1em" height="1em" fill="currentColor"><path d="M192 0C139 0 96 43 96 96V256c0 53 43 96 96 96s96-43 96-96V96c0-53-43-96-96-96zM64 216c0-13.3-10.7-24-24-24s-24 10.7-24 24v40c0 89.1 66.2 162.7 152 174.4V464H120c-13.3 0-24 10.7-24 24s10.7 24 24 24h72 72c13.3 0 24-10.7 24-24s-10.7-24-24-24H216V430.4c85.8-11.7 152-85.3 152-174.4V216c0-13.3-10.7-24-24-24s-24 10.7-24 24v40c0 70.7-57.3 128-128 128s-128-57.3-128-128V216z"/></svg>', title: "Rode NT1 5th Gen", descKey: "setupItem3Desc", asin: "B0BQH4KNBR" },
    { icon: '<svg data-fa="keyboard" class="icon fa-solid fa-keyboard" viewBox="0 0 576 512" width="1em" height="1em" fill="currentColor"><path d="M64 64C28.7 64 0 92.7 0 128V384c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H64zm16 64h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V144c0-8.8 7.2-16 16-16zM64 240c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V240zm16 80h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V336c0-8.8 7.2-16 16-16zm80-176c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H176c-8.8 0-16-7.2-16-16V144zm16 80h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H176c-8.8 0-16-7.2-16-16V240c0-8.8 7.2-16 16-16zM160 336c0-8.8 7.2-16 16-16H400c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H176c-8.8 0-16-7.2-16-16V336zM272 128h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H272c-8.8 0-16-7.2-16-16V144c0-8.8 7.2-16 16-16zM256 240c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H272c-8.8 0-16-7.2-16-16V240zM368 128h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H368c-8.8 0-16-7.2-16-16V144c0-8.8 7.2-16 16-16zM352 240c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H368c-8.8 0-16-7.2-16-16V240zM464 128h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H464c-8.8 0-16-7.2-16-16V144c0-8.8 7.2-16 16-16zM448 240c0-8.8 7.2-16 16-16h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H464c-8.8 0-16-7.2-16-16V240zm16 80h32c8.8 0 16 7.2 16 16v32c0 8.8-7.2 16-16 16H464c-8.8 0-16-7.2-16-16V336c0-8.8 7.2-16 16-16z"/></svg>', title: "Arturia KeyLab Essential 61 MkIII", descKey: "setupItem4Desc", asin: "B0D9DKHD35" },
    { icon: '<svg data-fa="volume-high" class="icon fa-solid fa-volume-high" viewBox="0 0 640 512" width="1em" height="1em" fill="currentColor"><path d="M533.6 32.5C598.5 85.2 640 165.8 640 256s-41.5 170.7-106.4 223.5c-10.3 8.4-25.4 6.8-33.8-3.5s-6.8-25.4 3.5-33.8C557.5 398.2 592 331.2 592 256s-34.5-142.2-88.7-186.3c-10.3-8.4-11.8-23.5-3.5-33.8s23.5-11.8 33.8-3.5zM473.1 107c43.2 35.2 70.9 88.9 70.9 149s-27.7 113.8-70.9 149c-10.3 8.4-25.4 6.8-33.8-3.5s-6.8-25.4 3.5-33.8C475.3 341.3 496 301.1 496 256s-20.7-85.3-53.2-111.8c-10.3-8.4-11.8-23.5-3.5-33.8s23.5-11.8 33.8-3.5zm-60.5 74.5C434.1 199.1 448 225.9 448 256s-13.9 56.9-35.4 74.5c-10.3 8.4-25.4 6.8-33.8-3.5s-6.8-25.4 3.5-33.8C393.1 284.4 400 271 400 256s-6.9-28.4-17.7-37.3c-10.3-8.4-11.8-23.5-3.5-33.8s23.5-11.8 33.8-3.5zM301.1 34.8C312.6 40 320 51.4 320 64V448c0 12.6-7.4 24-18.9 29.2s-25 3.1-34.4-5.3L131.8 352H64c-35.3 0-64-28.7-64-64V224c0-35.3 28.7-64 64-64h67.8L266.7 40.1c9.4-8.4 22.9-10.4 34.4-5.3z"/></svg>', title: "Yamaha HS8", descKey: "setupItem5Desc", asin: "B00CHZG75S" },
    { icon: '<svg data-fa="waveform" class="icon fa-solid fa-waveform" viewBox="0 0 640 512" width="1em" height="1em" fill="currentColor"><path d="M0 240c0-17.7 14.3-32 32-32s32 14.3 32 32v32c0 17.7-14.3 32-32 32s-32-14.3-32-32V240zm96-64c0-17.7 14.3-32 32-32s32 14.3 32 32V336c0 17.7-14.3 32-32 32s-32-14.3-32-32V176zm128-96c17.7 0 32 14.3 32 32V400c0 17.7-14.3 32-32 32s-32-14.3-32-32V112c0-17.7 14.3-32 32-32zm128 32c0-17.7 14.3-32 32-32s32 14.3 32 32V368c0 17.7-14.3 32-32 32s-32-14.3-32-32V112zm128 96c17.7 0 32 14.3 32 32v64c0 17.7-14.3 32-32 32s-32-14.3-32-32V240c0-17.7 14.3-32 32-32z"></path></svg>', title: "Ableton Live 12 Suite", descKey: "setupItem6Desc", asin: "B0CQXNY528", url: "https://www.amazon.com/Ableton-Live-Suite-Edition/dp/B0CQXNY528?th=1" }
  ];
  container.innerHTML = gear.map(g => `
    <a href="${g.url || 'https://www.amazon.' + dom + '/dp/' + g.asin + '?tag=topmusicg-20'}" target="_blank" rel="noopener noreferrer sponsored" class="setup-item" style="text-decoration:none;color:inherit">
...
function injectGuideJsonLd(guide) {
  ...
  const descText = guide.description || guide.description_es || intro.substring(0, 155);
  const article = {
    "@context": "https://schema.org", "@type": "Article",
    "headline": title,
    "description": lang === 'es' ? (guide.description_es || descText) : descText,
