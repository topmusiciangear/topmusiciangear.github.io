...
### ✅ CAMBIO 06/09/2026: fuente geolocalización = Cloudflare trace (rápida) + ipinfo solo de respaldo, SIN early return
**Reportado por el usuario:** con VPN México la home se quedaba clavada en zZounds (con VPN USA era correcto). Dos causas: (1) `quickTarget()` decide por la ZONA HORARIA del OS (el VPN no la cambia) → devolvía `zzounds`; (2) el SPA (`tmgGeoSwap` en `temp/gen-shop-buttons.js` → `js/shop-buttons.js`) tenía un **early return** (`if (q) { applyNow(q); return; }`) que impedía llegar a la resolución por IP → México nunca se detectaba. Además el usuario notó que en las guías el cambio a Amazon tardaba ~1-5s (latencia de ipinfo).
**Nuevo flujo (SPA + guías inline idéntico):** 1) `quickTarget()` (timezone) aplica al instante el candidato; 2) la red resuelve SIEMPRE: **Cloudflare trace** (`https://1.1.1.1/cdn-cgi/trace`, texto `loc=XX`, sin cuenta ni cuota, milisegundos) como **fuente primaria**; 3) si el trace falla/no da `loc` válido (ignora `XX`) → `gFallback()` corre **ipinfo.io/json** como **respaldo**. `applyNow(geoZone(cc))` se ejecuta una sola vez (guard `gres`): Cloudflare → país → zona (US→zzounds, GB→gear4music, países MS→musicstore, resto→amazon). **Se eliminó el early return del SPA** y todo `localStorage` de zona (`tmgGeoSwap`) para que la resolución de red corra en cada sesión fresca y la zona nunca quede obsoleta (el usuario descartó la memoria en navegador por ese riesgo). Helpers nuevos: `geoZone(c)` (SPA y inline) + `gApply`/`gFallback` (clausura). timeout 4000ms en ambos llamados. Regenerar `js/shop-buttons.js` con `node temp/gen-shop-buttons.js` y rebuild de guías (`node build-guides.js`) para el bloque inline.
...
- Resolución de red SIEMPRE corre (ya NO hay early return): `quickTarget` (timezone) aplica un candidato instantáneo que luego **afina** la red — **Cloudflare trace (`1.1.1.1/cdn-cgi/trace`) como primaria** e **ipinfo.io solo de respaldo** — ver regla "CAMBIO 06/09/2026" abajo. Sin `localStorage` de zona.
