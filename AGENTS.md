...
- **SESIÓN URLs zZounds reales para 138/145/372 (18/09/2026, segunda parte)**: las filas OOS de zZounds de X32 Compact (138), XR18 (145) y Genelec 8030C (372) ya NO enlazan al home — `cfg.urls.zzounds` añadidas vía `temp/oos_patch.json`: 138→`https://www.zzounds.com/productreview--BEHX32COMPACT`, 145→`https://www.zzounds.com/productreview--BEHXR18`, 372→`https://www.zzounds.com/item--GEN8030CP` (todas verificadas vivas con webfetch; los `item--` de 138/145 redirigen a home y `item--X32COMPACT` da 404; el item real de 372 existe como "No longer available at zZounds"). Aplicado con `temp/apply_patch.js` (ROUND-TRIP OK:true en 2ª pasada; la 1ª da false por orden de claves al añadir urls — no es corrupción), regen `js/shop-buttons.js` + `node build-guides.js` (300 páginas), verificado en guías (32 matches `productreview--`/`item--GEN8030CP`), pendiente deploy.
