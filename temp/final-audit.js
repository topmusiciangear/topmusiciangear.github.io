...
let total=0, missingUrl=0, missingPrice=0;
for(const prod of p){
  for(const k of stores){
    ...
    if(hasPrice && !hasUrl && !hasUrlTS) missingUrl++;
    if(hasUrl && !hasPrice) missingPrice++;
  }
}
console.log("Prices without URL:", missingUrl);
console.log("URLs without price:", missingPrice);
