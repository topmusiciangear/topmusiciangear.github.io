...
// Extract TEST_SHOP_BTN as a real object
...
// Serialize cleanly: each entry on one line, sorted by id
const lines=Object.keys(obj).sort((a,b)=>+a-+b).map(id=>{
  const entry=obj[id];
  const parts=[];
  if(entry.prices && Object.keys(entry.prices).length){
    parts.push("prices:"+JSON.stringify(entry.prices).replace(/"([^"]+)":/g,"$1:"));
  }
  if(entry.urls && Object.keys(entry.urls).length){
    parts.push("urls:"+JSON.stringify(entry.urls).replace(/"([^"]+)":/g,"$1:"));
  }
  if(entry.oos && entry.oos.length){
    parts.push("oos:"+JSON.stringify(entry.oos));
  }
  if(entry.na && entry.na.length){
    parts.push("na:"+JSON.stringify(entry.na));
  }
...
console.log("DONE - TEST_SHOP_BTN rewritten cleanly");
