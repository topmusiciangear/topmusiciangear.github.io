...
for(const k of stores){
  console.log(`${k}: ${missingByStore[k]?.length||0} products with price but no URL`);
  if(missingByStore[k]){
    missingByStore[k].slice(0,10).forEach(x=>console.log(`  ${x.id} | ${x.title} | ${x.price}`));
    if(missingByStore[k].length>10) console.log(`  ... and ${missingByStore[k].length-10} more`);
  }
}
