...
(async () => {
    ...
    try {
      ...
      const info = await page.evaluate(() => {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].match(/€\s*[\d,.]+/) && lines[i+1] && lines[i+1].includes('Each')) {
            return lines[i].trim();
          }
        }
        // Fallback: first EUR price
  ...
  // ALL products that I changed (from the 72 changes)
  const products = [
    [1, 'SM7B', '/Shure-SM7B/art-REC0000211-000'],
    [2, 'Neumann U87', '/Neumann-U87-Ai-Studio-Set/art-REC0009533-000'],
  ...
  for (const [id, name, slug] of products) {
    const url = 'https://www.musicstore.com/en_OT/EUR' + slug;
    const price = await getProductPrice(url);
    console.log(`ID:${id} | ${name} | ${price}`);
    results.push({ id, name, price });
  }
  
  fs.writeFileSync(path.join(__dirname, 'scraped-prices.json'), JSON.stringify(results, null, 2));
  console.log('\nResults saved to temp/scraped-prices.json');
