...
var patterns = {
  ththe: 'ththe', thany: 'thany', cleand: 'cleand', cthen: 'cthen',
  ...
  mG: ' mG ', xenyx: ' xenyx ', bd2: ' bD-2', pj: ' pJ ',
  stage4: ' stage 4 88', montage: ' montage m8x', xr18: ' xR18',
  cq18t: ' cQ-18t', tr8s: ' tR-8s', m32r: ' m32r LIVE', x32c: ' x32 compact',
  sq5: ' sQ-5', thisIt: ' this it', thisthe: 'This the ', thisthe2: ' this the ',
  athm50x: ' the ATH-M50x', mdr7506: ' the MDR-7506', ainnov: 'a innovative',
  cqis: 'The CQ-18T is a', instanten: ' al instantente', nth: 'thany .'
};
...
function show(node, path) {
  if (typeof node === 'string') {
    Object.keys(patterns).forEach(function (k) {
      ...
      if (idx >= 0) {
        var s = Math.max(0, idx - 40), e = Math.min(node.length, idx + p.length + 40);
        console.log('##' + k + ' :: ' + path);
        console.log(JSON.stringify(node.slice(s, e)));
      }
    });
