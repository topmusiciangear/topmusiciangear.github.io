...
// Find the icon value for in_ear_monitors in constants.js (uses old fa-ear-listen icon)
var j = c.indexOf("icon: '", i);
var k = c.indexOf("',\n", j + 7);
if (k === -1) k = c.indexOf("',desc:", j + 7);
var oldIcon = c.substring(j + 7, k);
console.log('Old icon in constants.js (first 80 chars):', oldIcon.substring(0, 80));
console.log('Old icon length:', oldIcon.length);

// Check if it was already replaced
if (oldIcon.indexOf('bi-earbuds') !== -1 || oldIcon.indexOf('fill-rule') !== -1) {
