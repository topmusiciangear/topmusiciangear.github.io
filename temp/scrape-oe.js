...
(async () => {
  ...
  for (const [id, name, slug] of products) {
    const price = await getProductPrice('https://www.musicstore.com/en_OE/EUR' + slug);
    console.log(`ID:${id} | ${name} | ${price}`);
    results.push({ id, name, price });
  }

  fs.writeFileSync('temp/oe-prices.json', JSON.stringify(results, null, 2));
  console.log('\nSaved to temp/oe-prices.json');
  await browser.close();
})();
