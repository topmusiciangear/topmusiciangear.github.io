...
for(const guide of g){
  if(guide.id && missing.includes(guide.id)){
    if(!guide.datePublished || guide.datePublished === "undefined" || new Date(guide.datePublished) > new Date()){
      guide.datePublished = today;
      console.log(`Updated ${guide.id} datePublished to ${today}`);
    }
  }
}

fs.writeFileSync(file, JSON.stringify(g,null,2));
console.log("DONE - dates updated");
