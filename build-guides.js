...
function guideCompControls(isEs) {
  ...
  var prog = '<div class="guide-comp-progress"><div class="guide-comp-progress-bar"></div></div>';
  return '<div class="guide-comp-controls">' + l + prog + r + '</div>';
}
  ...
  <main id="mainContent">
    ...
    <div class="guide-detail">
      ...
      ${guide.productTable ? (function(){
        ...
        return '<div class="guide-comp-wrap"><h2 class="guide-comp-title">' + (isEs && guide.productTable.title_es ? guide.productTable.title_es : (guide.productTable.title || (isEs ? 'Comparativa de Productos' : 'Product Comparison'))) + '</h2><div class="guide-comp-scroll-wrap"><div class="guide-comp-scroll"><table class="guide-comp-table"><thead><tr><th></th>' + headers + '</tr></thead><tbody>' + body + '</tbody></table></div>' + guideCompControls(isEs) + '</div></div>';
      })() : ''}
      ...
      <script>setTimeout(function(){document.querySelectorAll('.guide-product-card-desc').forEach(function(e){var b=e.nextElementSibling;if(b&&b.classList.contains('guide-product-card-desc-toggle')&&e.scrollHeight<=e.clientHeight)b.remove()});document.querySelectorAll('.guide-comp-wrap').forEach(function(w){var s=w.querySelector('.guide-comp-scroll');if(!s)return;var c=w.querySelector('.guide-comp-controls');if(c&&s.scrollWidth<=s.clientWidth+1)c.style.display='none'});document.querySelectorAll('.guide-comp-scroll').forEach(function(s){function upd(){var w=s.closest('.guide-comp-wrap');if(!w)return;var bar=w.querySelector('.guide-comp-progress-bar');if(bar){var max=s.scrollWidth-s.clientWidth;bar.style.width=(max>0?(s.scrollLeft/max*100):0)+'%'}}s.addEventListener('scroll',upd);upd()})},100)</script>
