...
const TEST_SHOP_BTN = {
  ...
  22: {prices:{amazon:"$1175.00",andertons:"£959.00",gear4music:"£812.00",musicstore:"€949.00"},oos:["zzounds"]},
  ...
  34: {oos:["zzounds"]},
  39: {prices:{andertons:"£42.00",gear4music:"£53.50",musicstore:"€168.00"},oos:["zzounds"]},
  ...
  64: {prices:{amazon:"$1,875.65",gear4music:"£1,799.00",musicstore:"€2,229.00"},oos:["zzounds","andertons"]},
  65: {prices:{amazon:"$862.39",zzounds:"$879.99",andertons:"£749.00",gear4music:"£739.00",musicstore:"€844.00"}},
  66: {prices:{amazon:"$849.99",gear4music:"£859.00",musicstore:"€1,090.00"},oos:["zzounds","andertons"]},
  ...
  92: {prices:{amazon:"$849.00"},urls:{gear4music:"https://www.gear4music.com/PA-DJ-and-Lighting/Sennheiser-EW-100-G4-Wireless-Microphone-System-with-935-S-E-Band/2BBJ",musicstore:"€99.00"},oos:["zzounds","andertons"]},
...
{prices:{amazon:"$749.00",andertons:"£599.00",gear4music:"£529.00",musicstore:"€503.36"}},
  ...
  139: {prices:{gear4music:"£3050.00",amazon:"$3,999.00",andertons:"£2,599.00",musicstore:"€2,990.00"},oos:["zzounds"]},
  ...
  148: {prices:{gear4music:"£2009.00",amazon:"$2,499.00",andertons:"£1,452.00",musicstore:"€2,099.00"},oos:["zzounds"]},
...
(function(){
  function doSwap(){
    ...
    if(curStore==='zzounds')return;
    var zRow=c.querySelector('[data-store="zzounds"]');
    var ml=c.querySelector('.shop-more-list');
    if(!zRow){
      if(!ml)return;
      var zHref='https://www.zzounds.com/item--'+encodeURIComponent((c.querySelector('.guide-product-card-title')||{}).textContent||'').replace(/%20/g,'+')+'?tag=topmusicg-20';
      var storeStyles2={zzounds:"font-family:'Poppins',Arial,sans-serif;font-weight:800;font-style:italic;color:#fff;letter-spacing:-.5px;font-size:15px"};
      var newP='<a href="'+zHref+'" target="_blank" rel="noopener noreferrer sponsored" class="shop-btn-primary" style="display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:0 16px;height:40px;border-radius:12px;background:#3b82f6;color:#fff;font-size:15px;font-weight:800;text-decoration:none;border:none;cursor:pointer;box-shadow:0 4px 16px rgba(59,130,246,.35);transition:box-shadow .2s ease,filter .2s ease,transform .18s ease" onmouseover="this.style.filter=\'brightness(1.05)\'" onmouseout="this.style.filter=\'\'"><span style="display:flex;align-items:center;gap:10px"><svg viewBox="0 0 576 512" width="1em" height="1em" fill="#fff" style="flex-shrink:0"><path d="M0 24C0 10.7 10.7 0 24 0L69.5 0c22 0 41.5 12.8 50.6 32l411 0c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3l-288.5 0-5.4 21.7c-1.1 4.5-.6 9.2 1.4 13.3L482.3 320l24 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-88 0c-30.9 0-56-25.1-56-56c0-25.9 17.6-47.6 41.5-53.9L442 128l-305.6 0c-14 26-33.1 60.1-44.4 81.5c-11 20.6-36.6 28.4-57.2 17.4c-20.6-11-28.4-36.6-17.4-57.2C35.7 133 63 82.9 74.5 61.8C83.5 45.1 100.9 34 120.8 34L96 34C82.7 34 72 23.3 72 20L0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"/></svg><span style="display:flex;align-items:center;gap:10px">Buy at<span style="'+storeStyles2['zzounds']+'">zZounds</span></span></span></a>';
      pb.insertAdjacentHTML('beforebegin',newP);
      pb.remove();
      return;
    }
